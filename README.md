...
- タイムライン上部の建物・定員・特別許可フィルターはlocalStorageに保存され、日付を移動しても維持されます。
- 予約枠を押すと詳細を確認でき、本人の予約は編集・キャンセルできます。
- 使用中の予約から相席・譲渡を申請でき、申請センターで承認・却下（却下理由必須）を処理できます。
- デバッグモードでも予約の重複、授業時間帯、4時間上限を検証し、本番と同じ申請・通知データをlocalStorageへ保存します。
